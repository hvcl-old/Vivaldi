...
template<typename R,typename T> GPU R point_query_2d(T* data, float2 p, int border, VIVALDI_DATA_RANGE* sdr){
	...
	int2 data_start = make_int2(sdr->data_start);
	int2 data_end = make_int2(sdr->data_end);

	int2 full_data_start = make_int2(sdr->full_data_start);
	int2 full_data_end = make_int2(sdr->full_data_end);
	
	int2 buffer_start = make_int2(sdr->buffer_start);
	int2 buffer_end = make_int2(sdr->buffer_end);
	...
	if( (!flag && (flag1 == 0 && flag2 == 0)) || (flag)){
		...
		int dif = buffer_halo - data_halo;
		// to Buffer coordinate 
		x = x - data_start.x + dif;
		y = y - data_start.y + dif;
...
template<typename R, typename T> GPU float2 linear_gradient_2d(T* image, float2 p, VIVALDI_DATA_RANGE* sdr){
	...
	int2 start = make_int2(sdr->data_start);
	int2 end = make_int2(sdr->data_end);
...
template<typename R, typename T> GPU R point_query_3d(T* image, float3 p, int border, VIVALDI_DATA_RANGE* sdr){
	int3 data_start = make_int3(sdr->data_start);
	int3 data_end = make_int3(sdr->data_end);

	int3 full_data_start = make_int3(sdr->full_data_start);
	int3 full_data_end = make_int3(sdr->full_data_end);
	
	int3 buffer_start = make_int3(sdr->buffer_start);
	int3 buffer_end = make_int3(sdr->buffer_end);
	...
	if( (!flag && (flag1 == 0 && flag2 == 0 && flag3 == 0)) || (flag)){
		...
		x = x - data_start.x + dif;
		y = y - data_start.y + dif;
		z = z - data_start.z + dif;
		
		rt = convert(image[z*Y*X + y*X + x]);
	}else{
		rt = initial2(image[0])*(0);
	}
	//rt = initial2(image[0])*(full_data_start.z+buffer_halo);
...
template<typename R, typename T> GPU float3 linear_gradient_3d(T* volume, float3 p, VIVALDI_DATA_RANGE* sdr){
	int3 start = make_int3(sdr->data_start);
	int3 end = make_int3(sdr->data_end);
...
extern "C"{
	...
	__global__ void writing_1d_uchar(uchar* rb, VIVALDI_DATA_RANGE* rb_DATA_RANGE,
	...
	__global__ void writing_1d_short(short* rb, VIVALDI_DATA_RANGE* rb_DATA_RANGE,
	...
	__global__ void writing_1d_int(int* rb, VIVALDI_DATA_RANGE* rb_DATA_RANGE,
	...
	__global__ void writing_1d_float(float* rb, VIVALDI_DATA_RANGE* rb_DATA_RANGE,
...
extern "C"{
__global__ void functionfloat(float* rb, VIVALDI_DATA_RANGE* rb_DATA_RANGE, float* data, VIVALDI_DATA_RANGE* data_DATA_RANGE, int x_start, int x_end, int y_start, int y_end){
    ...
    float val;
    float right;
    left = point_query_2d<float>(data, x-1, y, data_DATA_RANGE);
    right = point_query_2d<float>(data, x+1, y, data_DATA_RANGE);
    val = left && right;
    rb[(x-rb_DATA_RANGE->buffer_start.x)+(y-rb_DATA_RANGE->buffer_start.y)*(rb_DATA_RANGE->buffer_end.x-rb_DATA_RANGE->buffer_start.x)] = val;
    return;
