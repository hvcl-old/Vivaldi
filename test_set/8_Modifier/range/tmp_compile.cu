...
__global__ void function(int* rb, VIVALDI_DATA_RANGE* rb_DATA_RANGE, int x_start, int x_end, int y_start, int y_end){
    ...
    if(x_end <= x || y_end <= y)return;
    rb[(x-rb_DATA_RANGE->buffer_start.x)+(y-rb_DATA_RANGE->buffer_start.y)*(rb_DATA_RANGE->buffer_end.x-rb_DATA_RANGE->buffer_start.x)] = x + y;
    return;
